...
import api from "../api";
import SingleIssueTree from "./SingleIssueTree";
...
function IssueCard({ issue, renderMode }) {
    ...
    const checkPermissions = async () => {
      try {
        const likeResponse = await api.get(
          `/feedback/issues/${issue.id}/like-status/`
        );
        setIsLiked(likeResponse.data.liked);

        const deleteResponse = await api.get(
          `/feedback/issues/${issue.id}/delete-permission/`
        );
  ...
  // 森林模式现在使用 SingleIssueTree 组件
  // 注意：renderMode='forest' 时，父组件应直接传递 issues 数组
  // 这里保留单个 issue 卡片的旧版森林模式作为兼容
  ...
  // 新的单树模式：renderMode='single-tree' 时使用
  if (renderMode === "single-tree") {
    // 此时 issue 应该是 issues 数组
    return <SingleIssueTree issues={Array.isArray(issue) ? issue : [issue]} />;
  }
